...
namespace Quantum {
  ...
  public unsafe partial class Frame : Core.FrameBase {
    ...
    void SerializeState(FrameSerializer serializer) {
      ...
      SerializeAddonNavigationSdk(serializer);
    }
    ...
    /// </summary>
    /// <param name="systemType">System type to check</param>
    /// <returns><see langword="true"/> if the system was created during simulation start.</returns>
    public Boolean SystemExists(Type systemType) => _systemIndexByType.ContainsKey(systemType);
    ...
    /// <returns><see langword="true"/> if the system was created during simulation start.</returns>
    public Boolean SystemExists<T>() where T : SystemBase => _systemIndexByType.ContainsKey(typeof(T));
    ...
    /// Logs a warning if the system type is not found.
    /// </summary>
    /// <typeparam name="T">System type</typeparam>
    ...
    public Boolean SystemAllEnabledSelf(Type systemType) {
      if (_systemIndexByType.TryGetValue(systemType, out var systems)) {
        ...
        Log.Warn($"System '{systemType.Name}' not found, add to SystemsConfig or DeterministicSystemSetup.AddSystemsUser(), or check if SystemExists<T>() first.");
      }
    ...
    /// Logs an error if the systemRuntimeIndex is invalid.
    /// </summary>
    ...
    partial void SerializeAddonNavigationSdk(FrameSerializer serializer);
  ...
  public static partial class DebugCommand {
    ...
    public partial struct Payload {
      ...
      public override string ToString() {
        return $"[{nameof(Payload)} {nameof(Id)}={Id}, {nameof(Type)}={Type}, {nameof(Entity)}={Entity}, {nameof(Components)}={Components}, {nameof(Data)} Length: {Data?.Length ?? 0}, {nameof(SystemType)}={SystemType}, {nameof(SystemRuntimeIndex)}={SystemRuntimeIndex}]";
      }
    }
